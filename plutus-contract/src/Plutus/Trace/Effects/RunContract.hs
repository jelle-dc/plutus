...
import           Plutus.Trace.Emulator.Types (EmulatorThreads, EmulatorMessage(EndpointCall))
import           Plutus.Trace.Emulator.ContractInstance          (ContractInstanceError, contractThread, getThread)
import           Plutus.Trace.Scheduler (SystemCall, fork, Tag, Priority(..), mkSysCall, SysCall(Message))
...
data RunContract r where
    ActivateContract :: (ContractConstraints s, HasBlockchainActions s, Show e) => Wallet -> Contract s e a -> ContractInstanceTag -> RunContract (ContractHandle s e)
...
activateContractWallet :: forall s e effs. (HasBlockchainActions s, ContractConstraints s, Show e, Member RunContract effs) => Wallet -> Contract s e () -> Eff effs (ContractHandle s e)
