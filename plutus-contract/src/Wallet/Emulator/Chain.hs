...
data ChainEvent =
    TxnValidate Tx [ScriptValidationEvent]
    -- ^ A transaction has been validated and added to the blockchain.
    | TxnValidationFail Tx Index.ValidationError [ScriptValidationEvent]
...
instance Pretty ChainEvent where
    pretty = \case
        TxnValidate t _         -> "TxnValidate" <+> pretty (txId t)
        TxnValidationFail t e _ -> "TxnValidationFail" <+> pretty t <> colon <+> pretty e
...
mkValidationEvent t result events =
    case result of
        Nothing  -> TxnValidate t events
        Just err -> TxnValidationFail t err events
